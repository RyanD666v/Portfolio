...
const Footer = () => {
  ...
  return (
    <footer
      id='contact'
      className={cn(
        'bg-black relative px-12.5 w-full pt-30 overflow-hidden flex flex-col',
        isMobile ? 'pb-30' : ' pb-75  '
      )}
    >
      <div className='max-w-295 w-full mx-auto flex items-end gap-30 flex-wrap'>
        <h2 className='text-heading-2b text-cream w-95 wrap-break-word font-bold'>{t('footer.tagline')}</h2>
        <div className='flex items-start gap-30 flex-1 z-20 flex-wrap'>
      ...
      <div
        className={cn(
          'z-10 select-none w-[94%] max-w-300  flex items-center justify-center',
          isMobile ? 'relative' : 'absolute left-1/2 -translate-x-1/2 -bottom-35'
        )}
      >
        <p
          style={{
